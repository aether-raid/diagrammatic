...
const getNodeDescriptions = async (apiKey: string, nodeEdgeData: NodeEdgeData): Promise<NodeDescriptionData> => {
...
export const runNodeDescriptionsAlgorithm = async (nodes: AppNode[], nodeEdgeData: NodeEdgeData): Promise<AppNode[]> => {
  ...
  if (!apiKey) {
    vscode.window.showInformationMessage("Node descriptions are disabled. (No API key provided)");
  }

  const descriptions = apiKey ? await getNodeDescriptions(apiKey, nodeEdgeData) : undefined;
